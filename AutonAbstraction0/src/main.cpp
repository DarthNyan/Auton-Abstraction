...
//spin the motors for a number of degrees at an rpm
void DriveStraight(int dist, int speed)
{
  //clear encoders
  DriveLB.setPosition(0, degrees);
  DriveRB.setPosition(0, degrees);
  //use the average position of the drive halves
  while (0.5 * (DriveLB.position(degrees) + DriveRB.position(degrees)) < dist)
  {
    //spin the motors
    Drive(speed, speed);
  }
