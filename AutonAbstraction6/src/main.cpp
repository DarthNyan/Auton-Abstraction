...
void DriveStraight(float dist, float maxVel, float accel)
{
  ...
  if (dist > 0)
  {
    ...
    while (vel >= 0) 
    {
      ...
      //get the steering error
      currentSteeringError = currentHeading - gyroAbsPos() * DEG_TO_RAD;
      ...
      Drive(vel * INPERSEC_TO_RPM/ENCODER_TO_WHEEL //velocity in in/s converted to rpm
              ...
              + STEERING_KP * currentSteeringError, //p loop to correct steering
  ...
  else if(dist < 0)
  {
    ...
    while (vel <= 0) 
    {
      ...
      Drive(vel * INPERSEC_TO_RPM/ENCODER_TO_WHEEL //velocity in in/s converted to rpm
              + DRIVE_KP * (pos - lPosition) //p loop to keep near position target
              + STEERING_KP * currentSteeringError, //p loop steering correction
            vel * INPERSEC_TO_RPM/ENCODER_TO_WHEEL //right side is same as left
              + DRIVE_KP * (pos - rPosition)
              - STEERING_KP * currentSteeringError);
...
void DriveTurn(float radians, float outerRadius, float maxVel, float accel, bool reversed)
{
  //initialize variables
  ...
  if (radians > gyroStart)
  {
    while (outerVel >= 0)
    {
      ...
      if (reversed)
      {
        Drive( -outerVel * radiusRatio * INPERSEC_TO_RPM / ENCODER_TO_WHEEL //left wheel is inner wheel, negative now because reverse
                  -DRIVE_KP * (outerPos * radiusRatio + lPosition), //P loop to correct position
               -outerVel * INPERSEC_TO_RPM / ENCODER_TO_WHEEL //right wheel is outer wheel, negative because reversed
  ...
  else 
  {
    while (outerVel >= 0)
    {
      ...
      if (reversed)
      {
        Drive( -outerVel * INPERSEC_TO_RPM / ENCODER_TO_WHEEL //left wheel is outer wheel, negative because reversed
                  - DRIVE_KP * (outerPos + lPosition), //p loop to correct position
               -outerVel * radiusRatio * INPERSEC_TO_RPM / ENCODER_TO_WHEEL //right wheel is inner wheel, negative because reversed
                  - DRIVE_KP * (outerPos * radiusRatio + rPosition));
      }
      else
      {
...
//this will keept the robot in the direction it's facing for x seconds
void HoldHeading(int duration)
{
  ...
  for(int i = 0; i < duration * ticksPerSecond; i++)
  {
    ...
    // spin the motors
    Drive(STEERING_KP * currentSteeringError, -STEERING_KP * currentSteeringError);
    //standard loop time control
    currentTick += tickLength; 
    while(vexTimer < currentTick) {}
  }
